...
async function createSSHConnection(options) {
  ...
  if (options.password) {
    ...
    try {
      connectOptions.privateKey = await import_fs.promises.readFile(options.privateKeyPath);
    } catch (error) {
  ...
  await (0, import_events.once)(conn, "ready");
  return conn;
}
async function executeCommand(conn, command) {
  ...
  await (0, import_events.once)(stream, "close");
  if (stderr) {
